      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden lg:flex items-center space-x-1">
            ...
            {aiToolsItems.slice(0, 4).map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${isActive(item.path)
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <Link
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${isActive(user.role === 'doctor' ? '/doctor-dashboard' : '/patient-profile')
                      ? 'bg-primary text-primary-foreground'
                      : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                    }`}
                >
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-card rounded-lg mt-2 border border-border">
              ...
              {publicNavItems.map((item) => (
                <Link
                  ...
                  className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${isActive(item.path)
                      ? 'bg-primary text-primary-foreground'
                      : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                    }`}
                >
              ...
              {user ? (
                <>
                  <Link
                    to={user.role === 'doctor' ? '/doctor-dashboard' : '/patient-profile'}
                    onClick={() => setIsMenuOpen(false)}
                    className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${isActive(user.role === 'doctor' ? '/doctor-dashboard' : '/patient-profile')
                        ? 'bg-primary text-primary-foreground'
                        : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                      }`}
                  >
